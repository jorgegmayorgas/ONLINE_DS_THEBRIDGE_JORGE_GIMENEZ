...
def pinta_numericas (df:pd.DataFrame,columns:list=[],print_kde:bool=False):
    """
    Crea un hist plot comparando de dos en dos columnas
    

    Args:
    `df`(pd.DataFrame): DataFrame de pandas.
    `columns` (list): Lista con las columnas del DataFrame.
    `print_kde` (bool): Booleano para mostrar o no el KDE

    Retorna: None
    """
    if len(columns)<2:
        raise TypeError("Se necesita mínimo  una lista como mínimo con 2 columnas")
        return None
    for columna in columns:
        for columna_2 in columns:
            if columna_2!=columna:
                sns.histplot(df[columna], alpha=0.5, label=columna, kde=print_kde)
                sns.histplot(df[columna_2], alpha=0.5, label=columna_2, kde=print_kde)
                plt.xlabel('Value')
                plt.ylabel('Frequency')
                plt.title(f'Histograms: {columna} and {columna_2}')
                plt.legend()
                plt.show()
    return None